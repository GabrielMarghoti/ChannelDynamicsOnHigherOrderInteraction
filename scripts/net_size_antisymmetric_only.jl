...
# 2. Construtor de Rede (Apenas Antissimétrico)
...
function run_panel_sweep()
    ...
    # Tempos estendidos para garantir relaxação
    tspan = (0.0, 300.0)
    t_eq = 200.0
    ...
    for (n_idx, N) in enumerate(N_vals)
        println("Simulando N = $N (Antissimétrico)...")
    ...
    # 4. Plotagem (Painel Único)
    x_indices = 1:length(N_vals)
    x_ticks = (x_indices, string.(N_vals))
    
    plt = plot(size=(450, 300), xlabel=L"N", ylabel=L"R_q", ylims=(-0.05, 1.05), xticks=x_ticks, legend=:bottomleft)

    for n_idx in x_indices
        x_rep = fill(n_idx, num_trials)
        
        # Plotando distribuições com labels configurados para aparecer apenas uma vez na legenda
        violin!(plt, x_rep, R1_anti[n_idx, :], side=:left, color=:steelblue, alpha=0.8, width=0.4, linewidth=0, label=(n_idx==1 ? L"R_1" : ""))
        violin!(plt, x_rep, R2_anti[n_idx, :], side=:right, color=:forestgreen, alpha=0.8, width=0.4, linewidth=0, label=(n_idx==1 ? L"R_2" : ""))  
    end
    
    # Certifique-se de que o diretório "figures" existe antes de salvar
    mkpath("figures")
    savefig(plt, "figures/distribuicao_anti_tau$(τ).png")
    println("Figura salva: figures/distribuicao_anti_tau$(τ).png")
    return plt
end
